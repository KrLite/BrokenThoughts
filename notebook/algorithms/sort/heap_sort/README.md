...
堆排序的核心在于将待排序的列表看待为堆，进而将其转换为一颗完全二叉树[^完全二叉树]。
...
显而易见，对于这个长度为 `10` 的列表，我们可以将其转换为一个高度为 `4` 的堆， 其中列表的前 `5` 项在堆中均拥有至少一个子节点。

故而可以总结出：

1. 列表的前 $\lfloor \frac{n}{2} \rfloor$ 项在完全二叉树中均拥有至少一个子节点，意味着这些项为根结点或非叶子节点。
...
`heapify` 之所以叫做 `heapify（堆化）`，是因为它的作用是维持大顶堆的性质，即父节点的值大于其子节点的值。

有了 `heapify` 函数，我们就可以构建大顶堆了：
...
我们可以发现，根结点 `16` 必定拥有最大的值。目前的数组为：`[16, 14, 10, 8, 7, 9, 3, 2, 4, 1]`。

要想继续下一步排序，我们需要将第一项 `16` 与最后一项 `1` 进行交换，然后将已归位的最后一项（交换后为最大值 `16`）从堆中移除，再对新的跟节点进行 `heapify` 操作，以维持大顶堆的性质，以此不断保证靠后的项为更大值。

将 `16` 与 `1` 进行交换后，数组变为：`[1, 14, 10, 8, 7, 9, 3, 2, 4, 16]`，将最后一项 `16` 移除后，二叉树结构变为：
...
将 `1` 与 `14` 进行交换后，程序会继续对 `1` 进行 `heapify` 操作，得到：
...
将 `1` 与 `8` 进行交换后，程序会继续对 `1` 进行 `heapify` 操作，得到：
...
要完成全部元素的排序，我们只需要重复上述步骤直到堆中只剩下一个元素即可。
...
def heap_sort(arr: list) -> list:
	...
	for(i) in range(len(arr) - 1, 0, -1):	# 从最后一个元素开始，逐个向前交换
		swap(arr, 0, i)						# 将堆中的根结点（数组中的最大值）交换到最后
		arrLen -= 1							# 最大值归位，堆长度减一
		heapify(arr, 0)						# 对新的根结点进行 heapify 操作
