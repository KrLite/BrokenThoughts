...
const colorKeywordToRgbString = (keyword) => {
...
// Turn color into filter that is applicable to pure black contents
function execute() {
  const html = document.querySelector("body");
  const colorScheme = html.getAttribute("data-md-color-scheme");
  ...
  const colorName = getComputedStyle(root).getPropertyValue(variableName);
  const rgb = colorName.startsWith("#")
    ? colorName
    : rgbToNumbers(colorKeywordToRgbString(colorName));
